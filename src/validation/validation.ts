...
import * as vscode_languageserver_protocol from "vscode-languageserver-protocol"
...
export class validation{
    ...
    let serverJarPath = context.asAbsolutePath(path.join('server', 'org.eclipse.lsp4xml-all.jar'));
    ...
    //     function(res){
        if(javaPath){
            console.log('ibm ready');
            
            context.subscriptions.push(initLangClient(serverJarPath).start());
        }else{
            vscode.window.showWarningMessage("JAVA_HOME is not found in your Enviroment Path. Please install Java Jdk 1.8.0+")
        }
        // }
    // ).catch(function(rej){
    //     console.log(rej);
...
function initLangClient(serverJarPath){
