...
export function lsp(context: ExtensionContext) {
  let storagePath = context.storagePath;
  if (!storagePath) {
    storagePath = os.homedir() + "/.NativescriptExtend";
  }
  let logfile = path.resolve(storagePath + '/NativescriptExtend.log');
  ...
  function getXMLSettings(): JSON {
    let configXML = workspace.getConfiguration().get('NativescriptExtend');
    let xml;
...
function verifyAutoClosing() {
  ...
  let closeTags = configXML.get("NativescriptExtend.completion.autoCloseTags");
...
function verifyVMArgs() {
  let currentVMArgs = workspace.getConfiguration("NativescriptExtend.server").get("vmargs");
